...
public class RelatorioFuncionarioRegistro extends AbstractRelatorios{
	private FuncionarioService service;
	private List<Funcionario> lista;
	private String tituloFuncionario= "Relatório de Funcionários pelo Registro";
	private PdfPCell celulaCodigo, celulaNome, celulaCoren, celulaCidade;
	private float [ ] colunaLargura = { 1.50f , 5.50f , 4, 4 };
	
	public RelatorioFuncionarioRegistro(){
		service = new FuncionarioService();
		this.lista = service.BuscaRegistroCoren("CRM");
	}
	...
	protected void CriaTitulos() {
		...
		celulaCoren = new PdfPCell(new Paragraph("COREN/CRM/CRF", fonteCabecalho));
	...
	protected void PopularTabela() {
		for(Funcionario funcionario : lista){
			celulaCodigo = new PdfPCell(new Paragraph(funcionario.getCodigo()+"", fonte));				
